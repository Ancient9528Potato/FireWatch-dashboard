...
- **Signal panel** — the "watch this area" list. *(Definition folded back after mocking — see Spec Changelog.)* An area is flagged when its **most-recent-3-years mean annual fire count** is **≥ 1.3×** its **2006–2025 median annual fire count**; the same ratio is computed separately for total hectares, and both are shown (an area can be hot on count but not area, or vice-versa). The 1.3× threshold is the analyst-adjustable norm threshold referenced in Human-in-the-Loop.

**One stretch goal:**
- **Response-time view** — median lag from **`REPORTED_DATE` → `FIRE_FIGHTING_START_DATE`** by region (this exact pair; not dispatch→arrival), on the ~71% of fires with the second field, the 29% gap disclosed. *(Field pair pinned after mocking — see Spec Changelog: the panel was unfillable until the pair was named.)* Tells the analyst where suppression is slowest to start.
...
## Convergence — Three Candidates, Criteria Locked First

We locked the selection criteria **before** comparing concepts, so the data could not quietly choose for us.

**Criteria (locked first):**
1. **Real decision + owner** — a named user who can and will act on the output.
2. **Data can honestly carry it** — the claim survives the FPA-only, no-cost, no-forecast, right-skew constraints.
3. **Decision-changing, not just interesting** — moving the output changes where resources or effort go.
4. **Buildable and evaluable in the week** — and testable against known-answer cases.

**The three candidates, scored against the locked criteria:**
...
| B | Real-time operational monitor ("what's burning now") | Duty officer → live dispatch | ❌ data is a finalized annual record, **no live feed** | would, *if* data were live | ❌ can't — no live source | Rejected on C2 |
| C | Per-fire cause-investigation explorer (map every fire, drill to true_cause) | Investigator → individual case review | ⚠️ `TRUE_CAUSE` only 52.5% filled | weak — case-level, not a planning lever | ✅ | Rejected on C1/C3 |

**Why B was rejected (the one we wrote down):** It was the most tempting clever build, but it fails C2 outright — the data is a finalized annual record through 2025, not a live feed, so there is no "right now" to monitor. Building it would be a capable build aimed at a decision the data cannot serve. **Why C was rejected:** a per-fire map is interesting but operates at case level; the decision (where to concentrate crews and prevention) is regional, and its key field is half-null. We kept the user and the decision fixed, locked the criteria, and let the data veto B's framing — converging on **A**.

---

## Spec Changelog
*The spec is living: mocking the deliverable exposed gaps, which we folded back here rather than discovering them in the Build.*
...
1. **Watch-list signal was undefined.** The skeleton had a "running above norm" panel, but the spec never said *what* "above norm" meant — which window, count or hectares, what ratio. We could draw the box but not fill it honestly. **Folded back:** precise definition in Scope → Signal panel (recent-3-yr mean ≥ 1.3× the 2006–2025 median annual count; computed for count and hectares separately).
2. **Response-time panel was unfillable.** "Reported → first-suppression lag" did not name *which* timestamp pair (reported→fire-fighting-start vs dispatch→arrival), so the column had no defined number. **Folded back:** pinned to `REPORTED_DATE → FIRE_FIGHTING_START_DATE`, with the 29% missing-field share disclosed.

Both are now testable, and both surfaced *before* the Build — which is the point of mocking.
